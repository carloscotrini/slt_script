...
%
\begin{equation}
\exp\left(\frac{1}{T}c^\top c\right) = \left(\pi T\right)^{-d/2}\prod_{i \leq N}\int \exp\left(\frac{1}{T}y_i(c_i-y_i)\right) dy_i.
\end{equation}
%
Hint: Start here
%
...
&= \left(\pi T\right)^{-d/2}\sum_c \prod_i \int \exp\left(\frac{1}{T}y_i(c_i-y_i)\right) dy_i\\
&= \ldots \\
&= \left(\pi T\right)^{-d/2}\int \exp\left(-\frac{1}{T}\left(y^\top y - T \sum_{i \leq N} \log \left(2 \cosh\left(\frac{1}{T}y_i\right)\right)\right)\right) dy.\label{eq:norm_cost_ugly_integral}
\end{align}
%
Hint: Apply the combinatorial trick from Theorem~\ref{thm:gibbs_factorizes}. Recall also that
%
\begin{equation}
...
Show that, when $T > 1/2$, the conditions to apply Laplace's method hold. As a result, the integral in Equation~\ref{eq:norm_cost_ugly_integral} could be approximated to 1. Is this a good estimation?
\end{exercise}

It is still possible to apply Laplace's method when $T \leq 1/2$, and approximate the integral above to 1. However, doing this formally requires a more thorough understanding of the proof of Laplace's method.
